...
class OpenSearchService:
    def __init__(self):
        self.indices = ["events", "events-v2"]  # Search both indices
        ...
        self.client = AsyncOpenSearch(
            ...
            timeout=30,  # Increase default timeout
    ...
    async def verify_connection(self) -> bool:
        ...
        try:
            ...
            index_exists = await self.client.indices.exists(index=self.indices)
            if index_exists:
                logger.info(f"Index '{self.indices}' exists")
                return True
            else:
                logger.error(f"Index '{self.indices}' does not exist")
                return False
    ...
    async def search(self, query: Dict[str, Any], size: int = 0) -> Dict[str, Any]:
        ...
        logger.debug(f"Executing OpenSearch query: index={self.indices}, query={query}, size={size}")
    ...
    async def get_user_counts(self, start_date: datetime, end_date: datetime, event_name: str) -> Dict[str, int]:
        ...
        logger.info(f"Getting user counts for event: {event_name}, start_date: {start_date}, end_date: {end_date}")
        ...
        logger.debug(f"Using timestamp range: {start_ms} to {end_ms}")
        
        # Build query to match event name in both fields
        event_name_query = {
            "bool": {
                "should": [
                    {"term": {"event_name.keyword": event_name}},
                    {"term": {"event_name": event_name}},
                    {"term": {"name.keyword": event_name}},  # Try alternate field name
        ...
        query = {
            "query": {
                "bool": {
                    "must": [
                        event_name_query,
                        {"range": {self.timestamp_field: {"gte": start_ms, "lte": end_ms}}}
                    ]
                }
            },
            "aggs": {
                "users": {
                    "terms": {
                        ...
                        "size": 100000  # Increase size to handle more users
                    }
                }
            }
        }
        
        logger.debug(f"Executing OpenSearch query across indices: {self.indices}")
        ...
        try:
            ...
            response = await self.client.search(
                ...
                body=query,
                size=0,
                request_timeout=60  # Increase timeout for large queries
            )
            
            # Log the total hits for debugging
            total_hits = response.get("hits", {}).get("total", {}).get("value", 0)
            logger.info(f"Total hits for {event_name}: {total_hits}")
            
            # Extract user counts from aggregation buckets
            user_counts = {}
            if "aggregations" in response and "users" in response["aggregations"]:
                buckets = response["aggregations"]["users"]["buckets"]
                user_counts = {
                    bucket["key"]: bucket["doc_count"]
                    for bucket in buckets
                }
                logger.info(f"Found {len(user_counts)} unique users for event {event_name}")
                
                # Log some sample counts for debugging
                sample_counts = list(user_counts.items())[:5]
                logger.debug(f"Sample user counts: {sample_counts}")
            else:
                logger.warning(f"No aggregations found in response for event {event_name}")
    ...
    async def get_producers_count(self, date: Optional[datetime] = None) -> int:
        ...
        logger.info(f"Executing producers search query on index: {self.indices}")
        ...
        logger.debug(f"Executing OpenSearch query: index={self.indices}, query={query}, size=0")
